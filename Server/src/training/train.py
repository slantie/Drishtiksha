...
def run_training_loop(model, train_loader, val_loader, optimizer, device, config):
    ...
    if os.path.exists(checkpoint_path):
        ...
        history = json.load(open(history_path, 'r')) if os.path.exists(history_path) else {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': []}
    ...
    for epoch in range(start_epoch, train_config['num_epochs']):
        ...
        for i, (pixel_values, labels) in enumerate(train_progress):
            ...
            outputs = model(pixel_values, num_frames_per_video=model_config['num_frames']).squeeze()
            ...
            total_train_loss += loss.item() * train_config['accumulation_steps'] # Unscale for logging
