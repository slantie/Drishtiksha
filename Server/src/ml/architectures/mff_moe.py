...
class MFF_MoE(nn.Module):
    ...
    def load(self, path):
        ...
        # Filter out EMA-related keys that might not match
        filtered_state_dict = {}
        for k, v in state_dict.items():
            if not k.startswith('ema_list') and not k.startswith('ema_state'):
                filtered_state_dict[k] = v
        
        # Try to load with strict=False to handle potential mismatches
        try:
        ...
        if os.path.exists(ema_file):
            try:
                ...
                logger.warning(f"Failed to load EMA state: {e}. Continuing without EMA.")
        else:
            logger.warning(f"EMA state file not found: {ema_file}. Continuing without EMA.")

    def save(self, path, idx=0):
        torch.save(self.experts.state_dict(), os.path.join(path, 'MFF-MoE-v1.pth'))
        self.ema_state.update({idx: self.ema_list[idx].state_dict()})
        torch.save(self.ema_state, os.path.join(path, 'MFF-MoE-v1.state'))
