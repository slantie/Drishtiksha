...
from src.ml.utils import extract_frames, pad_frames, get_media_type
...
class BaseSiglipLSTMDetector(BaseModel):
    ...
    This class implements a unified `analyze` method that dispatches to the
    correct handler for video or image analysis.
    """
    ...
    def _analyze_image_patches(self, image: Image.Image, grid_size: int = 20) -> List[List[float]]:
        """Slices an image into a grid and analyzes each patch."""
        width, height = image.size
        patch_w, patch_h = width // grid_size, height // grid_size
        heatmap_scores = []
    ...
    def _analyze_image(self, image_path: str) -> ImageAnalysisResult:
        ...
        try:
            ...
            raise MediaProcessingError(f"Cannot identify image file: {image_path}")
        ...
        heatmap_scores = self._analyze_image_patches(image)
    ...
    def _analyze_video(self, media_path: str, **kwargs) -> VideoAnalysisResult:
        """
        The comprehensive analysis pipeline specifically for video files.
        """
        start_time = time.time()
        video_id = kwargs.get("video_id")
        user_id = kwargs.get("user_id")
        ...
        if not frame_scores:
            ...
            return VideoAnalysisResult(
                ...
                frame_count=total_frames, frames_analyzed=0,
                frame_predictions=[], metrics={}, visualization_path=None
            )

        # 4. Format frame-by-frame results into the standard schema
        frame_predictions = [
            FramePrediction(
                index=i, score=score, prediction="FAKE" if score > 0.5 else "REAL"
            ...
            event_publisher.publish(ProgressEvent(
                ...
                data=EventData(
                    ...
                    details={
                        "prediction": final_prediction, "confidence": final_confidence,
                        "processing_time": time.time() - start_time,
                        "total_frames": total_frames, "windows_analyzed": len(frame_scores)
                    }
                )
            ))
        ...
        return VideoAnalysisResult(
            prediction=final_prediction, confidence=final_confidence,
            processing_time=time.time() - start_time, note=note,
            frame_count=total_frames, frames_analyzed=len(frame_scores),
            frame_predictions=frame_predictions, metrics=metrics,
