...
class SiglipLSTMV1(BaseModel):
    ...
    def predict(self, video_path: str, **kwargs) -> Dict[str, Any]:
        ...
        return {"prediction": label_map[predicted_class_id], "confidence": confidence, "processing_time": processing_time}
class SiglipLSTMV3(SiglipLSTMV1):
    ...
    def _analyze_video_frames(self, video_path: str, video_id: str = None, user_id: str = None) -> Tuple[list[float], list[float]]:
        import cv2
    ...
    def predict_detailed(self, video_path: str, **kwargs) -> Dict[str, Any]:
        ...
        # CORRECTED: Ensure kwargs are passed down to enable event publishing.
        frame_scores, rolling_avg_scores = self._analyze_video_frames(video_path, video_id=video_id, user_id=user_id)
        ...
        # ADDED: Publish a final progress update for this stage.
        ...
        result = {
            ...
            "metrics": {
                "frame_count": len(frame_scores), "per_frame_scores": frame_scores,
                "rolling_average_scores": rolling_avg_scores, "final_average_score": avg_score,
                "max_score": max(frame_scores), "min_score": min(frame_scores),
    ...
    def predict_frames(self, video_path: str, **kwargs) -> Dict[str, Any]:
        ...
        frame_predictions = [{"frame_index": i, "score": score, "prediction": "FAKE" if score > 0.5 else "REAL"} for i, score in enumerate(metrics["per_frame_scores"])]
        return {
            ...
            "temporal_analysis": {"rolling_averages": metrics["rolling_average_scores"], "consistency_score": 1.0 - metrics["score_variance"]}
        }
