...
class EyeblinkDetectorV1(BaseModel):
    ...
    def predict_frames(self, video_path: str, **kwargs) -> Dict[str, Any]:
        ...
        # Scores are probabilities of being REAL, so prediction is REAL if score >= 0.5
        frame_predictions = [
    ...
    def predict_visual(self, video_path: str, **kwargs) -> str:
        ...
        try:
            for i in tqdm(range(total_frames), desc="Generating visualization"):
                ...
                # Map current video frame to a point on the sequence graph
                ...
                ax.clear()
                ax.plot(range(seq_len), sequence_scores, color="#4ECDC4", linewidth=2)
                if seq_len > 0:
                    ax.axvline(x=window_index, color='yellow', linestyle='--', linewidth=2)
                ax.axhline(y=0.5, color="white", linestyle="--", alpha=0.7)
                ax.set_ylim(-0.05, 1.05); ax.set_xlim(0, seq_len - 1 if seq_len > 1 else 1)
                ax.set_title("Blink Sequence Analysis (1.0=Real)", fontsize=10); fig.tight_layout(pad=1.5)
                fig.canvas.draw()
                ...
                new_h = int(frame_height * 0.3)
                new_w = int(new_h * (plot_w / plot_h))
                ...
                # Color bar: closer to 1.0 is green (REAL), closer to 0.0 is red (FAKE)
                ...
                cv2.putText(frame, f"Blink Sequence Score: {score:.2f} (1=Real)", (15, frame_height - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
                out.write(frame)
        finally:
            cap.release()
            out.release()
