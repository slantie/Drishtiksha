...
class LSTMDetector(BaseModel):
    ...
    def load(self):
        ...
        print(f"Loading LSTMDetector on device: {self.device}")
        ...
        try:
            ...
            # Load with map_location to ensure CPU loading and security
            ...
            print(
                f"✅ LSTMDetector '{self.config['name']}' loaded successfully on device '{self.device}'."
            )
    ...
    def predict(self, video_path: str):
        ...
        with torch.no_grad():
            logits = self.model(
                pixel_values, num_frames_per_video=self.config["num_frames"]
            )
    ...
    def predict_visualized(self, video_path: str) -> str:
        ...
        while cap.isOpened():
            ...
            cv2.rectangle(
                overlay,
                (x_offset, y_offset),
                (x_offset + new_plot_w, y_offset + new_plot_h),
                (0, 0, 0),
                -1,
            )
