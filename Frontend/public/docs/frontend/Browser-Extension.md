...
2.  **Authentication via Message Passing:**
    ...
    *   Instead, it sends a message (`{ type: "GET_AUTH_TOKEN" }`) to the `content.js` script, which is injected *only* into the pages of your active Drishtiksha web application.
