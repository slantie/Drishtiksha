...
**This is a critical manual step.** The model weight files are large and are not tracked in the Git repository. You must acquire the necessary model files (`.pth`, `.dat`, etc.) from the provided source (e.g., a shared Google Drive link, a zip file, a folder) and place them into the `Server/models/` directory.
...
![Model Integration](./Model-Integration.png)
...
![Containerization](./Model-Configurations.png)
