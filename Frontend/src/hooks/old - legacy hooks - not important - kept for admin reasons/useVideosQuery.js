...
export const useUploadVideoMutation = () => {
    ...
    return useMutation({
        ...
        onSuccess: (newVideoData) => {
            // The backend responds with the newly created video record.
            // We can add this new record directly to our cache.
            queryClient.setQueryData(queryKeys.videos.lists(), (oldData) => {
                if (!oldData || !Array.isArray(oldData.data))
                    return { data: [newVideoData.data] };
                return {
                    ...oldData,
                    data: [newVideoData.data, ...oldData.data],
                };
            });

            showToast.success("Video uploaded successfully!");
            showToast.info("Analysis has started in the background.");
        },
        onError: (error) => {
