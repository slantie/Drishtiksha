...
import { useRerunAnalysisMutation } from "../../hooks/useMediaQuery.jsx"; // Corrected hook path

export const RerunAnalysisModal = ({
  isOpen,
  onClose,
  media,
  onAnalysisStart,
  ...
  const handleConfirm = async () => {
    if (!media?.id) {
      console.error("RerunAnalysisModal: Media ID is missing.");
      onClose();
      return;
    }
    ...
    // This is useful for parent components that need to update their view
    // immediately after a new analysis run is queued.
    onAnalysisStart?.();
    onClose();
  };
  ...
  const modalFooter = (
    <>
      ...
      <Button
        ...
        disabled={!media?.id} // Disable if media ID is not available
      >
  ...
  return (
    <Modal
      ...
      description={`This will queue a new analysis for "${
        media?.filename || "this media file"
      }" using all currently available models.`}
      footer={modalFooter}
    >
