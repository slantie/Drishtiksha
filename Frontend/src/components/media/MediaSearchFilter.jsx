...
import {
  ...
  ShieldCheck,
  ShieldAlert,
} from "lucide-react"; // Added prediction icons
...
export const MediaSearchFilter = ({ mediaItems, onFilterChange }) => {
  const [filters, setFilters] = useState({
    ...
    status: "ALL",
    mediaType: "ALL",
    prediction: "ALL", // This will be 'REAL' or 'FAKE' based on overall assessment
  });

  // Notify the parent component whenever the filters change.
  // Use a deep comparison for filters to prevent unnecessary re-renders of the parent
  // or wrap the onFilterChange in useCallback in the parent.
  useEffect(() => {
      ...
      <CardContent className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 items-center">
            ...
            <SelectContent>
              ...
              {availableOptions.mediaTypes.map((type) => (
                <SelectItem key={type} value={type} className="capitalize">
                  {type === "VIDEO" && (
                    <FileVideo className="inline-block h-4 w-4 mr-2 text-purple-500" />
                  )}
                  {type === "AUDIO" && (
                    <FileAudio className="inline-block h-4 w-4 mr-2 text-blue-500" />
                  )}
                  {type === "IMAGE" && (
                    <FileImage className="inline-block h-4 w-4 mr-2 text-green-500" />
                  )}
                  {type.toLowerCase()}
                </SelectItem>
              ))}
          ...
          <Select
            ...
            onValueChange={(value) => handleFilterChange("prediction", value)}
          >
            <SelectTrigger>
            ...
            <SelectContent>
              ...
              {availableOptions.predictions.includes("REAL") && (
                <SelectItem value="REAL">
                  <ShieldCheck className="inline-block h-4 w-4 mr-2 text-green-500" />{" "}
                  Likely Authentic
                </SelectItem>
              )}
              {availableOptions.predictions.includes("FAKE") && (
                <SelectItem value="FAKE">
                  <ShieldAlert className="inline-block h-4 w-4 mr-2 text-red-500" />{" "}
                  Likely Deepfake
                </SelectItem>
              )}
            </SelectContent>
