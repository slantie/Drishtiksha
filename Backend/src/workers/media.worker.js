...
async function handleSingleAnalysis(job) {
    ...
    try {
        ...
        const resultPayload = await modelAnalysisService.runAnalysis(localMediaPath, modelName, mediaId, media.userId);
        ...
        logger.error(`[Worker] Analysis job for ${modelName} on media ${mediaId} failed: ${error.message}`, { stack: error.stack });
        if (media) {
        ...
        if (localMediaPath && isTempFile) {
            await fsPromises.unlink(localMediaPath).catch(err => 
                logger.error(`[Worker] Failed to clean up temp file ${localMediaPath}: ${err.message}`)
            );
        }
    }
}
...
async function handleFinalizeAnalysis(job) {
    ...
    if (!run) {
        logger.error(`[Finalizer] Cannot finalize, AnalysisRun ${runId} not found.`);
        return { status: 'error', reason: 'AnalysisRun not found.' };
    }

    const failedAnalyses = run.analyses.filter(a => a.status === 'FAILED').length;
    const finalStatus = failedAnalyses > 0 ? 'FAILED' : 'ANALYZED';

    await mediaRepository.updateRunStatus(run.id, finalStatus);
    await mediaRepository.update(mediaId, { status: finalStatus, latestAnalysisRunId: run.id });
    
    logger.info(`[Finalizer] Finalized AnalysisRun ${run.id} for media ${mediaId} with status: ${finalStatus}`);
...
async function _getLocalMediaPath(media) {
    ...
    if (config.STORAGE_PROVIDER === 'local') {
        const localPath = path.join(projectRoot, config.LOCAL_STORAGE_PATH, media.publicId);
        if (!existsSync(localPath)) throw new Error(`Local file not found at: ${localPath}`);
        return localPath;
    } else {
        const tempDir = path.join(projectRoot, 'temp', 'worker-downloads');
        await fsPromises.mkdir(tempDir, { recursive: true });
        const extension = path.extname(media.filename) || '.tmp';
        const tempFilePath = path.join(tempDir, `${media.id}-${Date.now()}${extension}`);
...
worker.on('failed', (job, err) => logger.error(`[Worker] Job '${job.name}' (ID: ${job.id}) has failed: ${err.message}`));
