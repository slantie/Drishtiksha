...
const uploadVideo = asyncHandler(async (req, res) => {
    ...
    // This service method now handles creating the DB record and adding the job to the queue.
...
const getModelServerStatus = asyncHandler(async (req, res) => {
    const status = await modelAnalysisService.getHealthStatus();
    res.status(200).json(
        new ApiResponse(
            200,
            status,
            "Model server status retrieved successfully."
        )
    );
});

const getAvailableModels = asyncHandler(async (req, res) => {
    const availableModels = await modelAnalysisService.getAvailableModels();
    res.status(200).json(
        new ApiResponse(
            200,
            { availableModels },
