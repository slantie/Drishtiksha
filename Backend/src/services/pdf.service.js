...
class PDFService {
  ...
  async generateRunReportPDF(analysisRunId, userId) {
    try {
      logger.info(
        `[PDF Service] Starting report generation for analysis run ${analysisRunId}`
      );

      // 1. Fetch analysis run data with media and analyses
      const analysisRunData = await this.getAnalysisRunData(
        analysisRunId,
        userId
      );
      ...
      logger.debug(
        `[PDF Service] Fetched analysis run data for media ${analysisRunData.id}`
      );

      // 2. Prepare data (enrich with calculations)
      const preparedData = this.prepareReportData(analysisRunData);
      logger.debug(
        `[PDF Service] Prepared report data with ${preparedData.completedAnalyses.length} completed analyses`
      );

      // 3. Generate markdown from template
      const markdown = this.generateMarkdownReport(preparedData);
      logger.debug(
        `[PDF Service] Generated markdown report (${markdown.length} characters)`
      );

      // 4. Convert markdown to PDF
      const pdfBuffer = await this.generatePDFFromMarkdown(markdown);
      logger.debug(
        `[PDF Service] Converted markdown to PDF (${pdfBuffer.length} bytes)`
      );
  ...
  async generatePDFFromMarkdown(markdown) {
    try {
      ...
      const pdf = await mdToPdf({ content: markdown }, this.pdfConfig);
  ...
  prepareReportData(media) {
    ...
    const overallAssessment =
      fakeDetections > realDetections ? "DEEPFAKE" : "AUTHENTIC";
    ...
    if (fakeDetections > 0) {
      if (
        fakeDetections >= completedAnalyses.length * 0.6 &&
        avgConfidence > 0.7
      ) {
        riskLevel = "HIGH";
...
**Analysis Date:** ${this.formatDate(
      data.latestAnalysisRun?.createdAt || new Date()
    )}
...
<div class="metadata-value"><span class="badge badge-${data.riskLevel.toLowerCase()}">${
      data.riskLevel
    } RISK</span></div>
</div>
...
<div class="metadata-value">${successRate}% (${data.completedAnalyses.length}/${
      data.totalModels
    })</div>
</div>
...
This comprehensive analysis examined a **${
      data.mediaType?.toLowerCase() || "media"
    }** file using ${
      data.totalModels
    } state-of-the-art deepfake detection models powered by advanced deep learning algorithms.
...
- **Consensus:** ${Math.round(
      (Math.max(data.realDetections, data.fakeDetections) /
        data.completedAnalyses.length) *
        100
    )}% model agreement
- **Average confidence:** ${confidencePercent}
- **Confidence range:** ${Math.round(
      data.lowestConfidence * 100
    )}% - ${Math.round(data.highestConfidence * 100)}%
- **Processing time:** ${this.formatDuration(
...
${
  data.failedAnalyses.length > 0
    ? `
**⚠️ Note:** ${
        data.failedAnalyses.length
      } model(s) failed during analysis. This may indicate:
...
The majority of detection models classified this content as authentic with ${
        data.avgConfidence > 0.8 ? "high" : "moderate"
      } confidence. ${
        data.confidenceSpread < 0.2
          ? "Model predictions show strong consistency."
          : "Some variation exists among model predictions, which is normal for authentic content."
      }
...
<div class="metadata-value">${
      duration > 0 ? this.formatDuration(duration) : "N/A"
    }</div>
</div>
...
<div class="metadata-value">${
      bitRate > 0 ? this.formatBitrate(bitRate) : "N/A"
    }</div>
</div>
...
<div class="metadata-value">${
        video.codecName || video.codecLongName || "N/A"
      }</div>
</div>
...
<div class="metadata-value">${
        video.bitRate && Number(video.bitRate) > 0
          ? this.formatBitrate(Number(video.bitRate))
          : "N/A"
      }</div>
</div>
...
<div class="metadata-value">${
        audio.codecName || audio.codecLongName || "N/A"
      }</div>
</div>
...
<div class="metadata-value">${
        audio.sampleRate
          ? `${Number(audio.sampleRate).toLocaleString()} Hz`
          : "N/A"
      }</div>
</div>
...
<div class="metadata-value">${audio.channels || "N/A"}${
...
<div class="metadata-value">${
        audio.bitRate && Number(audio.bitRate) > 0
          ? this.formatBitrate(Number(audio.bitRate))
          : "N/A"
      }</div>
</div>
...
The analysis pipeline processed the media through ${
      data.totalModels
  ...
  generateModelComparison(data) {
    ...
    const rows = allAnalyses
      .map((analysis) => {
        ...
        const modelName =
          analysis.modelName || analysis.model?.name || "Unknown Model";
        const modelType =
          analysis.modelType || analysis.analysisTechnique || "N/A";
...
<div class="page-break"></div>`;
    ...
    const modelDetails = data.completedAnalyses
      .map((analysis, index) => {
        const confidence = `${Math.round(analysis.confidence * 100)}%`;
        const badgeClass =
          analysis.prediction === "FAKE" ? "badge-deepfake" : "badge-authentic";
        const badgeText =
        ...
        let frameStats = "";
        if (hasFrameData) {
          const fakeFrames = framePredictions.filter(
            (f) => f.prediction === "FAKE"
          ).length;
          const realFrames = framePredictions.filter(
            (f) => f.prediction === "REAL"
          ).length;
          const avgFrameScore =
            framePredictions.reduce((sum, f) => sum + f.score, 0) /
            framePredictions.length;
          const maxFrameScore = Math.max(
            ...framePredictions.map((f) => f.score)
          );
          const minFrameScore = Math.min(
            ...framePredictions.map((f) => f.score)
          );
...
- Frames classified as REAL: ${realFrames} (${(
            (realFrames / framePredictions.length) *
            100
          ).toFixed(1)}%)
- Frames classified as FAKE: ${fakeFrames} (${(
            (fakeFrames / framePredictions.length) *
            100
          ).toFixed(1)}%)
- Average frame score: ${(avgFrameScore * 100).toFixed(2)}%
- Score range: ${(minFrameScore * 100).toFixed(2)}% - ${(
            maxFrameScore * 100
          ).toFixed(2)}%
- Confidence consistency: ${
            fakeFrames === 0 || realFrames === 0 ? "High" : "Variable"
          }`;
        }
...
${
  data.completedAnalyses.some(
    (a) => a.framePredictions && a.framePredictions.length > 0
  )
    ? `
...
**Total Duration:** ${this.formatDuration(
      (data.latestAnalysisRun?.duration || 0) / 1000
    )}
...
|-------|--------|---------|
| Model Execution | ${
      data.completedAnalyses.length === data.totalModels
        ? "✓ Complete"
        : "⚠ Partial"
    } | ${data.completedAnalyses.length}/${data.totalModels} completed |
| Failed Analyses | ${
      data.failedAnalyses.length === 0
        ? "✓ None"
        : `⚠ ${data.failedAnalyses.length}`
    } | ${data.totalRetries} retries |
...
**Confidential Analysis Report** | Generated by VidVigilante v2.0.0 | ${this.formatDate(
      new Date()
    )}
